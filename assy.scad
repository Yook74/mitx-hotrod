...
pwrbrd_hole_abs = [ 
    for (pos=power_board_hole_pos) 
        pos + [pwr_board_offset, pwr_board_space + d1 + power_board_hole_offset[1]]
];

mobo_hole_abs = [for (pos=mobo_hole_pos) pos + mobo_hole_offset]
...
translate([0, 0, -short_height]){
    for (pos=mobo_hole_abs) translate(pos) short_standoff();
    for (pos=pwrbrd_hole_abs) translate(pos) short_standoff();
}

for (pos=concat(pwrbrd_hole_abs, [mobo_hole_abs[2]], [mobo_hole_abs[3]])))
    translate(pos) translate([0, 0, mobo_pcb_thick]) tall_standoff();